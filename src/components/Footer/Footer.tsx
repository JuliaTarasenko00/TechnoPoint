...
const styleTitle = " text-[20px] font-semibold text-[#fff] leading-[110%]";

const styleLink = "text-[#cfcfcf] text-[14px] leading-[229%] hover:underline";
    ...
    <>
      <section className="container bg-[url('./../assets/image/Banner.png')] bg-cover w-full h-[448px] p-0 bg-center bg-no-repeat">
        <div className=" flex items-center justify-center flex-col w-full h-full">
          <p className=" text-[72px] tracking-[-0.01em] text-[#fff] font-thin">
          ...
          {information.map(({ title, svg: Image, description }) => (
            <li key={title} className=" flex gap-[2px] flex-wrap items-start">
              <div className="flex-1 flex gap-[5px] items-center">
                <span className=" w-[18px] h-[18px] block text-[#fff]">
                  <Image />
                </span>
                <h2 className=" text-[#fff] text-[12px]">{title}</h2>
              </div>
              <p className=" text-[#cfcfcf] text-[10px]">{description}</p>
            </li>
          ))}
        </ul>
      </section>
        ...
        <div className=" flex items-start justify-between">
          ...
          <div>
            ...
            <ul>
              {services.map((name) => (
                <li key={name}>
                  <a href={path.home} className={styleLink}>
                    {name}
                  </a>
                </li>
              ))}
          ...
          <div>
            <p className=" text-[16px] font-semibold text-[#fff] leading-[100%]">
          ...
          <div>
            ...
            <a
              href="tel:+0800000000"
              className=" text-[#fff] leading-[100%] text-[25px] my-[10px] block font-black"
            >
              0-800-000-000
            </a>
            <ul>
              {consultation.map(({ title, img: Image }) => (
                <li key={title} className="flex gap-[5px] items-center">
                  <span className=" w-[20px] h-[20px] block text-[#fff]">
                    <Image />
                  </span>
                  <a href={path.home} className={styleLink}>
                    {title}
